...
int main(){
    ...
    Klas cl;
    Student st;
    char ch, command[20], command2[20], gl[2], name[15], midle_name[15], surname[15], predmet[10];
    int nom, j=0, br=0, ocenka, ocenki[10];
    ...
    for(;;){
        br=0;
        fscanf(file, "%d %s %s %s %s", &nom, gl, name, midle_name, surname);
        ...
        for(int i=0; i<vip.v; i++){
            if(vip.vip_class[i].name==nom/100){
                ...
                while(!feof(file) && (ch=fgetc(file))!='\n'){
                    if(ch>='2' && ch<='6'){
                        vip.vip_class[i].st_class[vip.vip_class[i].k].Ocenki[vip.vip_class[i].st_class[vip.vip_class[i].k].o]=ch-'0';
                        (vip.vip_class[i].st_class[vip.vip_class[i].k].o)++;
                    }
                }
                (vip.vip_class[i].k)++;
                br=1;
        ...
        if(br==0){
            ...
            while(!feof(file) && (ch=fgetc(file))!='\n'){
                if(ch>='2' && ch<='6'){
                    vip.vip_class[vip.v].st_class[vip.vip_class[vip.v].k].Ocenki[vip.vip_class[vip.v].st_class[vip.vip_class[vip.v].k].o]=ch-'0';
                    (vip.vip_class[vip.v].st_class[vip.vip_class[vip.v].k].o)++;
                }
            }
    ...
    if((file=fopen("studentbyte", "wb"))==NULL){
        printf("error!");
        exit(1);
    }
    ...
    for(int i=0; i<vip.v; i++){
        for(int j=0; j<vip.vip_class[i].k; j++){
            fwrite(&vip.vip_class[i].st_class[j].num, sizeof(vip.vip_class[i].st_class[j].num), 5, file);
            fwrite(vip.vip_class[i].st_class[j].name, 1, strlen(vip.vip_class[i].st_class[j].name), file);
            fwrite(vip.vip_class[i].st_class[j].midle_name, 1, strlen(vip.vip_class[i].st_class[j].midle_name), file);
            fwrite(vip.vip_class[i].st_class[j].surname, 1, strlen(vip.vip_class[i].st_class[j].surname), file);
            for(int k=0; k<vip.vip_class[i].st_class[j].o; k++){
                fwrite(&vip.vip_class[i].st_class[j].Ocenki[k], sizeof(vip.vip_class[i].st_class[j].Ocenki[k]), 1, file);
            }
        fwrite("\n",1,strlen("\n"),file);
        }
    }
